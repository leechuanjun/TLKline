...
-(id)initWithUrl:(NSString*)url{
    if (self){
        // 取缓存的每天数据
        NSArray *tempArray = (NSArray*)[TLCommond getUserDefaults:@"daydatas"];
        if (tempArray.count>0) {
            self.dayDatas = tempArray;
        }
        NSArray *lines = (NSArray*)[TLCommond getUserDefaults:[TLCommond md5HexDigest:url]];
        if (lines.count>0) {
...
- (void)Finished:(ASIHTTPRequest *)request
{
    ...
    if ([self.req_type isEqualToString:@"d"]) {
        self.dayDatas = lines;
        [TLCommond setUserDefaults:lines forKey:@"daydatas"];
    }
    [TLCommond setUserDefaults:lines forKey:[TLCommond md5HexDigest:[[NSString alloc] initWithFormat:@"%@",request.url]]];
    [self changeData:lines];
    request = nil;
    self.isFinish = YES;
