...
//  TLKline
//
//  Created by lichuanjun on 15/9/27.
...
#import "TLGetStockData.h"
#import "TLCommond.h"

@interface TLLineView ()
{
    NSThread *thread;
    UIView *mainboxView; // k线图控件
    UIView *bottomBoxView; // 成交量控件
    TLGetStockData *stockData;
...
-(void)drawBox{
    ...
    if (!isUpdate) {
        ...
        for (int i = 0; i<7; i++) {
            ...
            TLLines *line = [[TLLines alloc] initWithFrame:CGRectMake(0, 0, mainboxView.frame.size.width, mainboxView.frame.size.height)];
            line.color = @"#333333";
...
-(void)drawLine{
    ...
    // 获取网络数据，来源于雅虎财经接口
    stockData = [[TLGetStockData alloc] init];
    stockData.kCount = self.kCount;
    stockData.req_type = self.req_type;
    stockData = [stockData initWithUrl:[self changeUrl]];
    self.data = stockData.data;
    self.category = stockData.category;
    NSLog(@"当前：%ld",(long)self.data.count);
    // 开始画K线图
    ...
    if (lineOldArray.count>0 && isUpdate) {
        for (TLLines *line in lineOldArray) {
...
-(void)changeMaxAndMinValue{
    CGFloat padValue = (stockData.maxValue - stockData.minValue) / 6;
    stockData.maxValue += padValue;
    stockData.minValue -= padValue;
}
...
-(void)drawBoxWithKline{
    ...
    for (int i = 0; i<7; i++) {
        ...
        leftTag.text = [[NSString alloc] initWithFormat:@"%.2f",padValue*i+stockData.minValue];
    ...
    // 开始画连K线
    // x轴从0 到 框框的宽度 mainboxView.frame.size.width 变化  y轴为每个间隔的连线，如，今天的点连接明天的点
    NSArray *ktempArray = [self changeKPointWithData:stockData.data]; // 换算成实际每天收盘价坐标数组
    TLLines *kline = [[TLLines alloc] initWithFrame:CGRectMake(0, 0, mainboxView.frame.size.width, mainboxView.frame.size.height)];
    ...
    // 开始画连成交量
    NSArray *voltempArray = [self changeVolumePointWithData:stockData.data]; // 换算成实际成交量坐标数组
    TLLines *volline = [[TLLines alloc] initWithFrame:CGRectMake(0, 0, bottomBoxView.frame.size.width, bottomBoxView.frame.size.height)];
    volline.points = voltempArray;
    volline.lineWidth = self.kLineWidth;
    volline.isK = YES;
    volline.isVol = YES;
    [bottomBoxView addSubview:volline];
    volMaxValueLab.text = [TLCommond changePrice:stockData.volMaxValue];
    [lineArray addObject:volline];
}
#pragma mark 画各种均线
-(void)drawMAWithIndex:(int)index andColor:(NSString*)color{
    NSArray *tempArray = [self changePointWithData:stockData.data andMA:index]; // 换算成实际坐标数组
    TLLines *line = [[TLLines alloc] initWithFrame:CGRectMake(0, 0, mainboxView.frame.size.width, mainboxView.frame.size.height)];
    line.color = color;
...
-(void)touchBoxAction:(UIPinchGestureRecognizer*)pGesture{
    isPinch  = NO;
    NSLog(@"状态：%ld==%f",(long)pinchGesture.state,pGesture.scale);
...
-(void)gestureRecognizerHandle:(UILongPressGestureRecognizer*)longResture{
    isPinch = YES;
    NSLog(@"gestureRecognizerHandle state:%ld",(long)longResture.state);
...
-(NSArray*)changePointWithData:(NSArray*)data andMA:(int)MAIndex{
    ...
    for (NSArray *item in data) {
        ...
        // 换算成实际的坐标
        CGFloat currentPointY = mainboxView.frame.size.height - ((currentValue - stockData.minValue) / (stockData.maxValue - stockData.minValue) * mainboxView.frame.size.height);
...
-(NSArray*)changeKPointWithData:(NSArray*)data{
    ...
    for (NSArray *item in data) {
        ...
        CGFloat yHeight = stockData.maxValue - stockData.minValue ; // y的价格高度
        CGFloat yViewHeight = mainboxView.frame.size.height ;// y的实际像素高度
        // 换算成实际的坐标
        CGFloat heightPointY = yViewHeight * (1 - (heightvalue - stockData.minValue) / yHeight);
        CGPoint heightPoint =  CGPointMake(PointStartX, heightPointY); // 最高价换算为实际坐标值
        CGFloat lowPointY = yViewHeight * (1 - (lowvalue - stockData.minValue) / yHeight);;
        CGPoint lowPoint =  CGPointMake(PointStartX, lowPointY); // 最低价换算为实际坐标值
        CGFloat openPointY = yViewHeight * (1 - (openvalue - stockData.minValue) / yHeight);;
        CGPoint openPoint =  CGPointMake(PointStartX, openPointY); // 开盘价换算为实际坐标值
        CGFloat closePointY = yViewHeight * (1 - (closevalue - stockData.minValue) / yHeight);;
...
-(NSArray*)changeVolumePointWithData:(NSArray*)data{
    ...
    for (NSArray *item in data) {
        ...
        CGFloat yHeight = stockData.volMaxValue - stockData.volMinValue ; // y的价格高度
        CGFloat yViewHeight = bottomBoxView.frame.size.height ;// y的实际像素高度
        // 换算成实际的坐标
        CGFloat volumePointY = yViewHeight * (1 - (volumevalue - stockData.volMinValue) / yHeight);
